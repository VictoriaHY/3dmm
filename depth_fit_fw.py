...
class Model(nn.Module):
    ...
    def forward(self, d=0, b1=0, b2=0, update=False):
        ...
        if b2>0:
            ...
            # print('m',self.m[:,:,:,0].sum())
            # print('m',self.m[:,:,:,1].sum())
            # pred_d = torch.zeros((b2-b1, 640, 360), dtype=torch.float32, device=self.device)
            ...
            #     pred_d[i,points1[i,:,1].long(), points1[i,:,0].long()] = points[i,:,2]#.double()
            
            points[:,self.num-1,2]=0
            pred_d = F.grid_sample(points[:,None,:,:], self.corresp[b1:b2, :,:,:], mode='nearest', align_corners=True)
            # print('max',pred_d.max())
            # print('points', points1[0,:3,:])
            # print('corres', self.corresp[0,points1[0,0,1].long(),points1[0,0,0].long(),1])
            # print(points[:,None,:,2].max())
            
            
            pred_d = pred_d#*self.m[b1:b2,None, : , :, 1]
            pred_d = pred_d.squeeze(1)
            # print
            # print(pred_d[0:10,315:325, 175:185])
            # print(ref_depth[0:10, 315:325, 175:185, 0]*self.m[0:10, 315:325, 175:185, 1])
            # print(self.m.shape)
            # print(pred_d.shape)
...
def fit_point(
    ...
    for i in epoch:
        ...
        if i<4000:
            ...
            loss, loss1, reg_loss, loss_d, R, T, key, m, _ = model(d=900, b1=0, b2=120, update=flag)
